 ...
 */
export async function PUT(req: Request, { params }: { params: Promise<{ volumePercent: number, deviceId: string }> }) {
  try {
    const { volumePercent, deviceId } = await params;
    ...
    if (!volumePercent || !deviceId) {
      return NextResponse.json({ error: "Données manquantes (volumePercent ou deviceId)" }, { status: 400 });
    }

    // console.log(`🎵 Modification du volume : ${volumePercent} sur device: ${deviceId}`);
