...
/**
 * API Route pour récupérer un morceau spécifique d'une playlist Spotify.
...
export async function GET(req: Request, { params }: { params: Promise<{ id: string, index: string }> }) {
  try {
    ...
    if (!id || isNaN(offset)) {
      return NextResponse.json({ error: "ID de playlist ou index invalide" }, { status: 400 });
    }
    ...
    const response = await fetch(
      `https://api.spotify.com/v1/playlists/${id}/tracks?fields=items(track(album(name,release_date,release_date_precision,images),artists(name),name,uri,id,href,popularity))&limit=1&offset=${offset}`,
      {
